...
| `VOLC_MODEL` | `doubao-seed-2-1-pro-260628` | 豆包语言模型的调用 ID（旧版本会下架，见下） |
...
> ⚠️ 其他注意：
> - **生成很慢，这是正常的**：实测约每个景点 15 秒 —— 1 天 4 个点 60 秒，5 天 20 个点 3 分 24 秒。
>   所以生成走流式（非流式会被负载均衡当僵死连接掐掉），`VOLC_TIMEOUT` 默认 600s。
> - 公网生成用的是**你的 key**＝别人也在花你的 token。配额是上限不是账单预测；
>   每张的真实 token 消耗会打在服务日志里（`⏱ 204.3s · token 输入 x / 输出 y`），按那个算账。
